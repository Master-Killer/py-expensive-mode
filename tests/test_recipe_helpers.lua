-- Tests unitaires pour lib/recipe-graph.lua, basés sur le framework LuaUnit.
-- Lancer avec : lua tests/test_recipe_helpers.lua [-v]
...
-- Réinitialise l'environnement de mock Factorio avant chaque test, pour que
-- les tests soient indépendants les uns des autres.
...
function TestRecipeGraph:setUp()
    reset_data()
end

function TestRecipeGraph:test_multiple_branches()
    pyem.recipe_graph({
...
function TestRecipeGraph:test_tanks_deep_graph()
    pyem.recipe_graph({
...
function TestRecipeGraph:test_chests_multi_chain()
    local chest_graph = {
...
function TestArmorPacking:setUp()
    reset_data()
    data.raw.armor = {
...
function TestArmorPacking:test_armor_chain_amounts()
...
function TestArmorPacking:test_packing_and_unpacking_recipes_registered()
    pyem.armor_recipe_chain({ "power-armor-mk2", "power-armor", "modular-armor", "heavy-armor", "light-armor" })

    lu.assertNotIsNil(data.raw.recipe["power-armor-packing"], "Packing recipe not found")
    lu.assertNotIsNil(data.raw.recipe["power-armor-unpacking"], "Unpacking recipe not found")
end
